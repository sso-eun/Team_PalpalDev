    ...
    private fun fetchGuardianAndSeniorInfo() {
        viewModelScope.launch {
            try {
                ...
                // 상대경로("profile/...")면 절대 URL로 보정
                guardianProfileImg = toAbsoluteUrl(
                    RetrofitClient.apiBaseUrl,
                    guardianResponse.userProfileImg
                )
                ...
                // 필요 시 절대 URL 보정
                seniorProfileImg = toAbsoluteUrl(
                    RetrofitClient.apiBaseUrl,
                    seniorResponse.userProfileImg
                ) ?: ""
                ...
                if (context != null) {
                    ...
                    if (success) {
                        Log.d(TAG, "어르신 일정 로드 성공")
                    } else {
                        Log.w(TAG, "어르신 일정 로드 실패")
                    }
                }
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    guardianTel = newTel
                    // 화면 표시용 절대 URL로 보정
                    guardianProfileImg = toAbsoluteUrl(RetrofitClient.apiBaseUrl, finalProfileImgPath)
                    onSuccess()
                } else {
